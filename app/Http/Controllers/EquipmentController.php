    ...
    public function removeEquipmentOrder(Request $request) {
        ...
        if ($existingReservationDetail) {
            if ($existingReservationDetail->quantity > 1) {
                ...
                $existingReservationDetail->save();
                return response()->json(['success' => true, 'message' => 'Quantity reduced']);
            } elseif ($existingReservationDetail->quantity == 1) {
                $existingReservationDetail->delete();
                return response()->json(['success' => true, 'message' => 'Item removed from reservation']);
            } else {
                return response()->json(['error' => 'Quantity is already 0'], 400);
            }
        } else {
